...
\texttt{Treatment}. Each sample series is then tracked over time:
...
In the figure above, the \texttt{linetypeF} highlights samples from an
animal which start at unusually high expression values, while they
progressively return to expression values similar to other samples in their
respective treatment groups.

If omitted, the \texttt{linetypeF} argument will match the colour

may be omitted for clarity if the identity of each
