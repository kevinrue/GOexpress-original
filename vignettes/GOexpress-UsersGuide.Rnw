...
not supported yet, the clustering of non-transcriptomics datasets (e.g.
...
This toy \Rclass{ExpressionSet} also includes a \Rclass{AnnotationDataFrame}
...
\subsubsection[Prepare the grouping factor to analyse]{Prepare the grouping
factor to analyse}
...
\subsubsection[Run the random forest algorithm using local annotations]{Run
the random forest algorithm using local annotations}

Now, we use the random forest statistical framework to score each gene
feature on its
ability to cluster samples from different treatments separately,
before summarising this information at the ontology level. In this
...
to generate suitable local annotations the benefits of using them).
...
If none of the annotations are provided, \Rfunction{GO\_analyse} will
...
This step is the longest step in the pipeline, but merely takes a minute
(using locally saved annotations)
to analyse approximately 12,000 genes in 117 samples with default
...
\warning{Without local annotations, roughly another minute would be spent
contacting the Ensembl server and downloading annotations from the current
annotation release.}
...
type of ontology).
To optimse the use of memory space, after removing all ontologies not passing
the criteria, the function will also discard from the filtered result object
all gene features not associated
with the remaining gene ontologies, and the mapping information related to
gene ontologies absent from the filtered data. 
However, those data will still be left in the object containing unfiltered
raw results. Both raw and filtered objects can be provided to the
visualisation functions described later in this User's Guide.

In the above raw results, all three types (i.e. namespaces) of
...
partially corrects for that bias.
...
An alternative to the above cascade filtering of gene ontologies and gene
features, users can extract and filter information from either the
...
Note that this will return a data-frame containing only the filtered
GO terms, as opposed to the full list object returned by the
...
<<fig=FALSE>>=
heatmap_GO(
    go_id = "GO:0034142", result = BP.5, eSet=AlvMac, f='Group',
    cexCol=1, cex.main=1, main.Lsplit=30)
@
\end{center}

Note that since release 1.1.6 it is also possible to control independently
the re-labelling of sample names, and the colour-coding factor:

\begin{center}
<<fig=FALSE>>=
AlvMac$Treatment.Time <- paste(AlvMac$Treatment, AlvMac$Time, sep="_")
heatmap_GO(
    go_id = "GO:0034142", result = BP.5, eSet=AlvMac,
    cexCol=1, cex.main=1, main.Lsplit=30,
    labRow=AlvMac$Treatment.Time)
@
\end{center}
...
The default behaviour of the function is to order the gene features
...
The function \Rfunction{subEset} takes care of this task, updating 

takes a named list, where item
...
rapidly identify those with gene expression data best clustering the samples
...
Special thanks to Dr. Kate Killick and Dr. Karsten Hokamp
for testing and feedback. My thanks to the
