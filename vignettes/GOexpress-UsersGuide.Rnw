...
\bioctitle{GOexpress: identify and visualise robust gene ontology
signatures through supervised clustering of gene expression data}
...
graphics and summary tables. In short, the \Biocpkg{GOexpress} package is
a software package
...
each gene in each sample. Commonly, this expression matrix is filtered to
...
partially controls that bias.

However, in our experience, this bias presents some benefits. First, it
...
We attempt to maintain a running version on a small webserver:
\url{http://animalgenomics.ucd.ie:3838/AlvMac/}}
...
Special thanks to Dr. Paul Cormican, Simone Coughland, Dr. Karsten Hokamp,
and an unknown reviewer for feedback leading to some features. 
Heartily thanks to Dr. Kate Killick for testing on human data and feedback. 
