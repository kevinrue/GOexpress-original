...
\title{GOexpress: Visualise microarray and RNAseq data using gene ontology
annotations}
\author{Kévin Rue-Albrecht, Paul A. McGettigan, Belinda Hernández,\\
Andrew Parnell, Stephen V. Gordon, and David E. MacHugh}
...
\subsection[The origin and purpose of \texttt{GOexpress}]{The origin and
purpose of \texttt{GOexpress}}

The idea leading to the \texttt{GOexpress} R package emerged from a set of
...
\texttt{GOexpress} package and discussing its features with colleagues and
...
genewise visualisation is left to the end-user. However, data summarisation and
visualisation can be a daunting task in multifactorial experiments, or require
large amounts of copy-pasting to investigate the expression profile of a
handful or genes and cellular pathways.

Tested on multiple RNA-seq and microarray datasets, \texttt{GOexpress} offers
...
expression. Once the initial analysis and filtering of \texttt{GOexpress}
...
graphics. In short, the \texttt{GOexpress} package is a software package
...
Notably, the use of the \texttt{biomaRt} package enables \texttt{GOexpress} to
support and annotate gene expression matrix from any species and any
microarray platform present in the Ensembl BioMart:
...
implemented in the \texttt{GOexpress} package, as well as a tutorial taking the
user through a typical analysis pipeline that \texttt{GOexpress} was designed
for. While an example usage will be provided for each function of the package,
the many arguments of each function cannot realistically be demonstrated in
this Guide, and we kindly ask users to also read the individual R vignettes
...
Installing \texttt{GOexpress} should be as easy as running the two lines below:
...
The \texttt{GOexpress} package is still at an early stage of development and
may require some fine-tuning or yet undetected bug fixes. Please contact the
maintainer with a copy of the error message and the command run.

Despite our efforts to repeatedly test the \texttt{GOexpress} package on
...
BioMart have not been tested yet. Do contact the package maintainer if you feel
that the error is on our part:
...
As for all Bioconductor packages, the Bioconductor developers mailing list is
probably the best place to seek advice with a large and active community of
Bioconductor users \texttt{http://www.bioconductor.org/help/mailing-list/}.

\subsection[Citing \texttt{GOexpress}]{Citing \texttt{GOexpress}}

The work underlying \texttt{GOexpress} has not been formally published yet. A
manuscript is in preparation and will be submitted to a bioinformatics journal
in due course. In the meantime, users of the package are encouraged to cite
the \texttt{GOexpress} package using the \texttt{citation} method of the
\texttt{utils} package, as follows:
...
\texttt{GOexpress} requires this prefiltered normalised expression matrix to be
accompanied by an \texttt{AnnotatedDataFrame} object of the \texttt{Biobase}
package providing phenotypic information for each of those samples (e.g. unique
identifier, treatment, time-point). \texttt{GOexpress} expects those two
variables in an \texttt{ExpressionSet} container of the \texttt{Biobase}
package, both simplifying the manipulation of the data and, most importantly,
ensuring interoperatibility with other packages that handle Bioconductor
\texttt{ExpressionSet} objects. The other fields of the \texttt{ExpressionSet}
container may be left empty as \texttt{GOexpress} does not currently access
them.

To use the analytic part of the \texttt{GOexpress} package, the phenotypic
data-frame --- \texttt{phenodata} slot of the \texttt{ExpressionSet} --- must
...
programming language. The above \texttt{ExpressionSet} and the name of the
column containing such a factor are the minimal two input variables required
for the \texttt{GO\_analyse} method to work. Additional arguments may be
required, in particular for microarray datasets, but those are discussed later
in section \ref{sec:MicroarrayArguments}.

In the examples below, we will use the sample dataset \texttt{AlvMac} provided
...
\texttt{GOexpress} packge as well:
...
Now, the expression matrix and phenotypic data of the \texttt{ExpressionSet}
container can be access using dedicated methods from the \texttt{Biobase}
package:
...
An advantage of the \texttt{ExpressionSet} container is that it takes care of
...
\texttt{GOexpress}, namely that the expression matrix must have row names made
of unique and supported feature identifiers (i.e. microarray probeset or
...
Note that in the training dataset, the \texttt{Time} column of
\texttt{pData(targets)} is an R factor while the \texttt{Timepoint} column is
...
while respecting the relative distance between the time-points.

\subsection[Main analysis]{Main analysis}

In this example, we search for GO terms enriched in genes best clustering the
samples according their \texttt{Treatment} level. But first, let us make sure
that the \texttt{Treatment} column of \texttt{pData(targets)} is indeed an R
factor:

<<>>=
is.factor(pData(AlvMac)$Treatment) # assertion test
pData(AlvMac)$Treatment # visual inspection
@

In this case, it is already a properly formatted factor. If that was not the
case, the following line of code would convert the column to an R factor and
allow to continue the analysis (note that in some cases, it may be preferrable
to order the different levels of a factor, for an example see factor
\texttt{Time}):

<<>>=
pData(AlvMac)$Treatment <- factor(pData(AlvMac)$Treatment)
@
...
method will score each GO term by aggregating the average rank of its
associated genes:

<<>>=
raw_results <- GO_analyse(eSet = AlvMac, f = "Treatment")
@

Note that the above command does not specify which species the data originated
from. As mentioned in the output messages, the first feature identifier in the
...
(e.g. in the prefix `ENSBTA', `BT' indicates \textit{Bos taurus}).

\label{sec:MicroarrayArguments}
However, it can be more difficult to identify the microarray used to obtain a
certain dataset, as many Affymetrix chips contain probesets named with the
pattern `AFFX.*'. In cases where the microarray platform cannot be detected
automatically, we recommend users to use the \texttt{microarray} argument of
the \texttt{GO\_analyse} method. The list of valid values for the microarray
argument is available in the \texttt{microarray2dataset} data frame which can
...
and can easily be browsed with standard R methods:

<<>>=
names(raw_results) # Data slot names
str(raw_results) # Details of data slots
head(raw_results$GO, n=5) # Ranked table of GO terms (subset)
head(raw_results$genes, n=5) # Ranked table of genes (subset)
head(raw_results$mapping) # Gene to gene ontology mapping table (subset)
@
...
GO terms are merged in a single ranked table. It is however possible to extract
the GO terms corresponding to a single namespace. 

<<>>=
BP <- subset_scores(result = raw_results,
    namespace = "biological_process")
MF <- subset_scores(result = raw_results,
    namespace = "MF") # alias for "molecular_function"
CC <- subset_scores(result = raw_results,
...
<<>>=
BP.5 <- subset_scores(result = raw_results,
    namespace = "biological_process",
    total = 5) # requires 5 or more associated genes
MF.10 <- subset_scores(result = raw_results,
    namespace = "molecular_function",
    total = 10)
CC.15 <- subset_scores(result = raw_results,
...
\texttt{total} filter. Note that this filter retains only GO terms associated
with a minimal given count of genes in the Ensembl BioMart. It is also possible
to use the \texttt{data} argument to filter for GO terms associated with a
certain count of genes in the given expression dataset, although this approach
is obviously more data-dependent and less robust.
...
of the factor considered (\texttt{raw\_results\$factor}).
...
of samples by \texttt{Group} (i.e. combination of treatment and time-point)
...
``\texttt{toll-like receptor 4 signaling pathway}'' gene ontology:
...
Similarly to the \texttt{heatmap\_GO} method above, among the various arguments
of the \texttt{cluster\_GO} function, the argument \texttt{f} can be used to
specify an alternative column present in \texttt{pData(targets)} to label the
samples in the dendrogram according to that phenotypic information. Obviously,
if the same set of genes is being used, the clustering of samples will remain
the same no matter the column chosen; however, the alternative labelling of
samples may reveal relevant sub-clustering of samples.
...
from the Ensembl BioMart. For obvious reasons, genes present in the BioMart but
absent from the expression dataset will be absent from the score table and
consequently lack annotations. It is possible to restrict the above table to
only genes present in the expression dataset using the \texttt{data.only}
argument.
...
Note that "\texttt{Timepoint}" is another column of \texttt{pData(targets)}.
That column does not encode a factor, but a numeric vector. This difference
...
To investigate the impact of other factors on the expression level of the same
gene, users are encouraged to use the \texttt{f} and \texttt{x\_var} arguments
to specify alternate grouping factor and X variable, respectively. Note that
the \texttt{geom\_smooth} of the \texttt{ggplot2} package may fail if a minimal
number of replicates is not available to calculate proper confidence intervals.
Here is another valid example separating samples by animal on the X axis.
...
associated gene names if any. This is a more human-friendly version of the
...
the lattice. Subsequently, each of the sub-figures plotted may be re-plotted by
itself using the \texttt{index} argument as indicated in the accompanying
...
identify outliers within sample groups, or visually compare paired samples, for
instance.

In the \texttt{AlvMac} dataset, samples from each of the animals were
subjected to all three treatments in parallel (i.e. paired samples). In the
figure below, a sample series is defined by a given \texttt{Animal} and a given
\texttt{Treatment}. Each sample series is then tracked over time, and coloured
according to the \texttt{Treatment} factor (default, factor stored in
\texttt{raw\_results\$factor}):

\begin{center}
<<fig=TRUE>>=
AlvMac$Animal.Treatment = paste(AlvMac$Animal, AlvMac$Treatment, sep="_")
expression_profiles(gene_id = "ENSBTAG00000047107", result = raw_results,
...
In the figure above, the \texttt{linetypeF} helps to highlight samples from an
animal which start at unusually high expression values, while those samples
progressively return to expression values similar to other samples in their
respective treatment groups.

If omitted, the \texttt{linetypeF} argument will mirror the \texttt{colourF},
which can be useful for colour-blind people. Alternatively, a single line-type
can be applied to all groups using the \texttt{argument} as follows:
...
Similarly to the \texttt{expression\_plot} method, an alternative was
implemented to use gene names instead of Ensembl gene identifiers. An example:

<<fig=TRUE>>=
expression_profiles_symbol(gene_symbol="TNIP3", result = raw_results,
                    linetypeF="Animal", line.size=1.5, eSet=AlvMac,
                    seriesF="Animal.Treatment", x_var = "Timepoint",
...
the accompanying \texttt{phenoData} on the gene expression in the different
groups of samples. 
...
terms of genes, which limits the number of genes identified by those approaches
and blurring the precise cellular function represented by the core gene-set
among a list of closely related yet different functions.
...
filename argument is left to the default \texttt{NULL} value (if a filename is
...
This method calls the \texttt{venn.diagram} method of the \texttt{VennDiagram}
...
statistics produced by \texttt{GOexpress}. The distribution of scores may be
...
functions. Therefore, the default behaviour of \texttt{GOexpress} is to use the
rank and average rank metrics to order genes and GO terms, respectively, in the
returned score tables.
...
\texttt{GOexpress} was initially created from a set of gene-based, and later
ontology-based, visualisation functions. Following the integration of those
various plotting functions at the core of the \texttt{GOexpress} package, the
need for a ranking of genes and GO terms soon became apparent in order to
rapidly identify those best clustering the samples according to the
experimental factor studied, resulting in the clearest dendrograms and
heatmaps. A two-fold procedure was implemented:
\begin{enumerate}
    \item Using the available expression data, each gene present in the dataset
    is scored, evaluating its ability to cluster the predefined groups of
    samples. Genes are ranked according to their respective score; ties are
    resolved by assigning the lowest rank R to all G genes, giving the rank G+n
    to the next gene(s). The genewise scoring functions implemented are
    described in the following subsections, and all were validated on in-house
    datasets cross-checked with widely validated analytic pipelines (e.g.
    Ingenuity\textregistered Pathway Analysis, SIGORA)
    \item Using the above ranks and scores, each GO term in the Ensembl BioMart
    is assigned the mean score and the mean rank of all the genes associated
    with it in the BioMart. Genes present in the BioMart but absent from the
    given dataset are assigned a score of 0 (minimum valid score; indicates no
    power to discriminates the predefined groups of sample) and a rank equal to
    the number of genes present in the entire dataset plus one (worst rank,
    while preserving discrete continuity of the ranking).
\end{enumerate}
...
clustering trees and gene expression profiles are purely driven by the
expression data, sample phenotype annotations provided and GO terms ontologies
imported from the Ensembl BioMart server, without any transformation applied to
the data. Therefore, users are encouraged to use and suggest alternative
...
does each gene in the dataset cluster predefined groups of samples?''. Given a
number of trees (dendrograms) to build using the samples in the dataset and a
number of genes to sample from the dataset to build those trees, the
\texttt{randomForest} R package evaluates the ``importance'' of each gene in
the clustering of the predefined groups of samples; genes clustering sample
well will have a higher importance than genes randomly clustering samples from
the various groups.  Technically, \texttt{GOexpress} extracts the
\texttt{MeanDecreaseGini} value from the \texttt{importance} slot of the
\texttt{randomForest} output and uses this value as the score for each gene.
This score cannot be less than 0 (random clustering). The higher the mean
decrease in Gini, the greater the power of the corresponding gene to partition
the data into the predefined groups, with fewer ``out-of-bag'' (i.e.
incorrectly clustered) samples.
...
genes sampled. Indeed, at each node in the dendrogram genes are sampled from
the expression data and tested for their individual capacity to improve the
partitioning reached using a particular gene set. For combinations of genes
clustering well the predefined groups of samples, all the relevant genes will
be given an accordingly high importance (i.e. score). The larger the number of
trees and genes sampled, the more complete the coverage of interactions will
be.
...
compared to the variance within the groups, summarised in as an F statistic
that \texttt{GOexpress} uses as a score for each gene. Simply put, if samples
...
will produce a higher score. This score cannot be less than 0 (variance between
groups insignificant to variance within groups), while very large ratios can
easily be reached for genes markedly different between groups.
...
taken care of by \texttt{GOexpress}. Indeed, the \texttt{oneway.test} method of
the package \texttt{stats} is used with parameter \texttt{var.equal} set to to
\texttt{FALSE}. While this reduces the sensitivity of the test, all genes are
affected by this correction based on the relative amount of variance in the
different predefined groups of samples.
...
Conception and development of the \texttt{GOexpress} package was carried out
...
AP. This User's Guide was prepared and edited by KR-A, PAM, BH, AP, SVG, and
DEM.

\subsection[Acknowledgement]{Acknowledgments}

Special thanks to Dr. David Magee for helpful advice on statistics, to Dr.
Nicolas Nalpas and Dr. Kate Killick for testing and feedback. My thanks to the
Bioconductor staff and in particular to Hervé Pagès for the helpful feedback
which improved the standards of the code and documentation. Last but not least,
thanks to the University College Dublin "OpenSequencing" and the "Virtual
Institute of Bioinformatics and Evolution" (VIBE) communities where I first
presented raw versions of \texttt{GOexpress} and got valuable feedback and
advice in return, underlying a significant number of updated and new features.
