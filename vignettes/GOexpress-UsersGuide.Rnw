...
However, if everything else fails, the \texttt{GOexpress} package has been maintained on a GitHub repository since the dawn of the project. The source code, precompiled binaries for Windows and Unix, a Wiki, and a bug tracking system can be found at:
...
The development of this package would not have been possible without the support and encouragement (and datasets) of my co-supervisors Prof. Stephen Gordon and Prof. David MacHugh. I would like to thank Dr. Andrew Parnell and Belinda Hernandez who suggested and helped me integrate the random forest framework in my package. Special thanks to Dr. Paul McGettigan for the many tips and tricks which made R an easier world for me, and Dr. David Magee for helpful advice on statistics. Last but not least, thanks to the Virtual Institute of Bioinformatics and Evolution (VIBE) community where I first presented \texttt{GOexpress} version 0.5.1 and got valuable feedback and advice underlying a significant number of new and updated features.
