...
expression_plot = function(gene_id, expr_data, phenodata, x_var, result, f=result$factor, 
  ...
  gg = ggplot2::ggplot(df) +
    ggplot2::geom_smooth(aes(x=X, y=Expression, group = Factor, color = Factor, fill=Factor), level=level) +
    ggplot2::labs(title = title) +
    ggplot2::xlab(x_var) +
    ggplot2::ylab(ylab) +
    ggplot2::theme(plot.title = ggplot2::element_text(size = ggplot2::rel(title.size))) +
...
expression_plot_symbol = function(gene_symbol, expr_data, phenodata, x_var, result,
  ...
  # However, we still don't know how many of those identifiers are present in the expression
  # dataset. Remove the ensembl identifiers absent from our dataset as we cannot plot them
...
heatmap_GO = function(go_id, result, expr_data, phenodata, gene_names=TRUE,
                      ...
                      trace="none", expr.col=gplots::bluered(75), row.col.palette="Accent",
                      row.col=RColorBrewer::brewer.pal(n=length(unique(Biobase::pData(phenodata)[,f])), name=row.col.palette),
                      main=paste(go_id, result$GO[result$GO$go_id == go_id,"name_1006"]),
                      ...){
  # Fetch the list of genes associated with the go_id
  gene_ids = GOexpress::list_genes(result, go_id, data.only=TRUE)
  ...
  gplots::heatmap.2(genes_expr, labRow=sample_labels, labCol=gene_labels, scale=scale,
...
subset_scores = function(result, ...){
  ...
  for (filter in names(list(...))){
    ...
    ## Filters on the namespace of the GO term
      ...
      if (filters[filter] %in% c("biological_process", "BP")){
        ...
        filtered[,filter] = result$GO$namespace_1003 == "molecular_function"
      }
      else if (filters[filter] %in% c("cellular_component", "CC")){
        filtered[,filter] = result$GO$namespace_1003 == "cellular_component"
      }
      else{
  ...
  # Filter the different slots of results to remain consistent between them and save memory
  ...
  result$genes = result$genes[rownames(result$genes) %in% result$mapping$gene_id,]
  return(result)
}
...
table_genes = function(result, go_id, data.only=FALSE){
  ...
  gene_ids = GOexpress::list_genes(result=result, go_id=go_id, data.only=data.only)
